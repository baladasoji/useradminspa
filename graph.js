...
function assignUserToGroup (element, url, userid, groupid)
{
	url = `/${groupid}/members/$ref`;
	user = `{ "@odata.id": "https://graph.microsoft.com/v1.0/directoryObjects/${userid}" }` ;
...
{
	var testgrp =  allgroups.filter(function(grp) { return grp.Env === "CDT" ; }) ;
	var stagegrp =  allgroups.filter(function(grp) { return grp.Env === "Staging" ; }) ;
	var prodgrp =  allgroups.filter(function(grp) { return grp.Env === "Production" ; }) ;
...
function getUserWithEmail()
{
	...
	callGraphApi('userresult',"users?$filter=eq(mail,\'" + mailaddress+ "\')&$select=id,mail",access_token);
