...
{
    if (getSubGroups(CDTGroups, "CDT", "package").includes(groupid))
    {
	//alert ('got a package group');
	sgrp = getSubGroups(CDTGroups, "CDT", "included") ;
	for (grp in sgrp)
	{
    ...
    var included=false;  
    if (getSubGroups(CDTGroups, "CDT" , "included").includes(groupid))
    {
	included = true;
    }
	userid = user.id;
    ...
    apiXMLReq.onreadystatechange = function() {
		...
		if (!included)
		{
		    showMsgNSecs('alert-info','Removing group from user <i class="fa fa-spinner fa-spin" style="font-size:24px"></i>', 5);
		    setTimeout(function() 
		    { 
			checkUserGroups();
		    }, 5000);
...
//	alert ('got a package group');
    ...
    apiXMLReq.onreadystatechange = function() {
		...
		if (!included)
		{
		showMsgNSecs('alert-info','Adding group to user <i class="fa fa-spinner fa-spin" style="font-size:24px"></i>', 4);
		    setTimeout(function() 
		    { 
			checkUserGroups();
		    }, 4000);
...
//	alert('I am getting called');
	...
	sortOutGroups();
        ...
        //callRolesApi('userroles','roles', sessionStorage.access_token);
        //callUserApi('username','username', sessionStorage.access_token);
